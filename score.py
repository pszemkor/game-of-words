...
class ScoreCounter:
    def __init__(self, board, newly_added=None,
                 tiles_with_fixed_neighbours=None):
        self.board = board
    ...
    def __check_fixed_neighbour(self, pos, board):
        ...
        for i in range(len(x_dirs)):
            ...
            if 0 <= x_neigh < config.BOARD_SIZE and \
                    0 <= y_neigh < config.BOARD_SIZE:
                if board.fields[x_neigh][y_neigh].state == \
                        model.FieldState.FIXED:
                    return True
        return False
    ...
    def __check_whether_has_vertical_neighbours(self, pos):
        if pos[0] + 1 < config.BOARD_SIZE and \
                self.board.fields[pos[0] + 1][pos[1]].state == \
                model.FieldState.FIXED:
            return True
        if pos[0] - 1 >= 0 and self.board.fields[pos[0] - 1][pos[1]].state == \
                model.FieldState.FIXED:
            return True
        return False

    def __check_whether_has_horizontal_neighbours(self, pos):
        if pos[1] + 1 < config.BOARD_SIZE and \
                self.board.fields[pos[0]][pos[1] + 1].state == \
                model.FieldState.FIXED:
            return True
        if pos[1] - 1 >= 0 and self.board.fields[pos[0]][pos[1] - 1].state == \
                model.FieldState.FIXED:
            return True
        return False
    ...
    def horizontal_word_score(self, pos):
        ...
        if pos[1] + 1 < config.BOARD_SIZE and \
            ...
            for y in range(pos[1] + 1, config.BOARD_SIZE):
                if self.board.fields[pos[0]][y].state == \
                        model.FieldState.EMPTY:
                    break
    ...
    def vertical_word_score(self, pos):
        ...
        if pos[0] + 1 < config.BOARD_SIZE and \
            ...
            for x in range(pos[0] + 1, config.BOARD_SIZE):
                if self.board.fields[x][pos[1]].state == \
                        model.FieldState.EMPTY:
                    break
    ...
    def count_score(self):
        ...
        if self.tiles_with_fixed_neighbours is None:
            self.tiles_with_fixed_neighbours = \
                self.__get_tiles_with_fixed_neighbours()

        score = 0
        horizontal_sorted = sorted(self.newly_added, key=lambda x: x[1])
        vertical_sorted = sorted(self.newly_added, key=lambda x: x[0])
        if (len(horizontal_sorted) == 1 and
            self.__check_whether_has_horizontal_neighbours(
                horizontal_sorted[0])) \
                or (len(horizontal_sorted) > 1 and horizontal_sorted[0][0] ==
                    horizontal_sorted[1][0]):
            # horizontal word which consists of newly-added letters
            ...
            for y in range(horizontal_sorted[0][1], -1, -1):
                ...
                elif self.board.fields[x][y].state == \
                        model.FieldState.TEMPORARY:
                    score += self.board.fields[x][y].tile.get_value() * \
                             self.__count_letter_bonus((x, y))
                else:
                    break
            if len(self.newly_added) > 1:
                for y in range(horizontal_sorted[0][1] + 1,
                               horizontal_sorted[::-1][0][1]):
                    if self.board.fields[x][y].state == model.FieldState.FIXED:
                        score += self.board.fields[x][y].tile.get_value()
                    elif self.board.fields[x][y].state == \
                            model.FieldState.TEMPORARY:
            ...
            for y in range(horizontal_sorted[::-1][0][1], config.BOARD_SIZE):
                if self.board.fields[x][y].state == model.FieldState.FIXED:
                    ...
                    break
            if len(self.newly_added) == 1:
                score -= self.board.fields[x][horizontal_sorted[::-1][0][1]].\
                             tile.get_value() * \
                             self.__count_letter_bonus((x,
                                                        horizontal_sorted
                                                        [::-1][0][1]))
            ...
            for pos in self.tiles_with_fixed_neighbours:
                if self.__check_whether_has_vertical_neighbours(pos):
                    score += self.vertical_word_score(pos) \
                             + self.board.fields[pos[0]][pos[1]].\
                             tile.get_value() \
            ...
            for x in range(vertical_sorted[0][0], -1, -1):
                if self.board.fields[x][y].state == model.FieldState.FIXED:
                    ...
                    break
            if len(self.newly_added) > 1:
                for x in range(vertical_sorted[0][0] + 1,
                               vertical_sorted[::-1][0][0]):
                    if self.board.fields[x][y].state == model.FieldState.FIXED:
                        score += self.board.fields[x][y].tile.get_value()
                    elif self.board.fields[x][y].state == \
                            model.FieldState.TEMPORARY:
            ...
            for x in range(vertical_sorted[::-1][0][0], config.BOARD_SIZE):
                if self.board.fields[x][y].state == model.FieldState.FIXED:
                    ...
                    break
            if len(self.newly_added) == 1:
                score -= self.board.fields[vertical_sorted[::-1][0][0]][y].\
                            tile.get_value() * \
                            self.__count_letter_bonus(
                                (vertical_sorted[::-1][0][0], y))
            ...
            for pos in self.tiles_with_fixed_neighbours:
                if self.__check_whether_has_horizontal_neighbours(pos):
                    score += self.horizontal_word_score(pos) + \
                             self.board.fields[pos[0]][pos[1]].\
                             tile.get_value() \
                             * self.__count_letter_bonus(pos)
                    score *= self.__count_word_bonus(pos)
